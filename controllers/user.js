...
userController.get('/find/:userId', async (req, res) => {
  try {
    ...
    if (!user) {
      console.log('No such user, wrong id')
...
userController.delete('/deleteUser/:userId', verifyToken, async (req, res) => {
  if (req.params.userId === req.user.id) {
    ...
    console.log('You can delete only your own profile')
    return res.status(500).json({ msg: 'You can delete only your own profile' })
  }
})
// follow/unfollow
...
userController.put('/toggleFollow/:otherUserId', verifyToken, async (req, res) => {
  try {
    ...
    //if we don't follow user, we want to folow him, otherwise we unfollow him
    if (!currentUser.following.includes(otherUserId)) {
      ...
      return res.status(500).json({ msg: "You have successfully followed the user" })
    } else {
      currentUser.followings = currentUser.followings.filter((id) => id !== otherUserId)
      otherUser.followers = otherUser.followers.filter((id) => id !== currentUser)
      await currentUser.save()
      await otherUser.save()
